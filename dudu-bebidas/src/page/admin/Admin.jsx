...
const EMPTY_PRODUCT = {
  id: "", name: "", category: "cerveja", price: "", old_price: "",
  image: "", stock: "", is_active: true, promotion: false, supplier: "", ean: "",
};

// ── Helpers ───────────────────────────────────────────
const isPickup     = (order) => order.address?.isRetirada === true;
const getStatusMap = (order) => isPickup(order) ? STATUS_PICKUP : STATUS_DELIVERY;
const getConfig    = (order) => getStatusMap(order)[order.status] ?? STATUS_DELIVERY.pending;
...
function useVariableVirtualList(count, estimatedItemHeight = 90, overscan = 3) {
  ...
  let start = 0;
  while (start < count - 1 && offsets[start + 1] <= scrollTop - threshold) start++;
  let end = start;
  while (end < count - 1 && offsets[end] <= scrollTop + viewH + threshold) end++;
...
export default function Admin({ user, isAdmin }) {
  ...
  const [metrics,      dispatchMetrics] = useReducer(metricsReducer, { count: 0, total: 0 });
  ...
  const handleModalSave = async (e) => {
    ...
    else { await fetchProducts(); setProductModal(null); }
  ...
  const fetchTodayMetrics = useCallback(async () => {
    ...
    if (!error && data)
      dispatchMetrics({ count: count ?? 0, total: data.reduce((s, o) => s + (o.total ?? 0), 0) });
  }, []);
  ...
  const confirmReject = useCallback(async () => {
    ...
    const { data: rpcResult, error: rpcError } = await supabase.rpc("restore_stock", {
      p_order_id: rejectModal,
    });

    if (rpcError || !rpcResult?.success) {
      setRejectError(rpcError ? "Erro ao restaurar estoque." : (rpcResult?.error ?? "Erro ao restaurar estoque."));
      setRejecting(false);
      return;
    }
  ...
  const closeRejectModal = useCallback(() => {
    if (!rejecting) { setRejectModal(null); setRejectError(""); }
  }, [rejecting]);
          ...
          <>
            ...
            <div className="adm-modal adm-modal-product" role="dialog" aria-modal="true">
              ...
              <form onSubmit={handleModalSave} className="adm-product-form">
                ...
                <div className="adm-form-field">
                  <label>Nome</label>
                  <input name="name" value={modalForm.name} onChange={handleModalChange}
                  ...
                  <div className="adm-promo-fields">
                    ...
                    <div className="adm-form-row">
                      <div className="adm-form-field">
                        ...
                        <input name="old_price" type="number" step="0.01" min="0"
                          value={modalForm.old_price ?? ""} onChange={handleModalChange} placeholder="ex: 10.00" />
                      </div>
                      <div className="adm-form-field">
                        <label>Preço promocional (R$)</label>
                        <input name="price" type="number" step="0.01" min="0"
                          value={modalForm.price} onChange={handleModalChange} placeholder="ex: 7.50" required />
                      </div>
                    </div>
                ...
                <div className="adm-modal-actions">
                  <button type="button" className="adm-modal-btn-back"
                    onClick={() => setProductModal(null)} disabled={modalSaving}>Cancelar</button>
            ...
            <div className="adm-title-row">
              <div>
                ...
                <p className="adm-subtitle">
                  {totalCount} pedido(s) · mostrando {orders.length} · tempo real
                </p>
              </div>
        ...
        {activeTab === "produtos" && (
          <>
            <div className="adm-title-row">
              ...
              <button className="adm-btn-new-product" onClick={openNewProduct}>+ Novo Produto</button>
            </div>

            <div className="adm-product-filters">
              <input className="adm-product-search" placeholder="🔍 Buscar produto..."
                value={productSearch} onChange={(e) => setProductSearch(e.target.value)} />
              <select className="adm-product-cat-filter"
                    ...
                    {filteredProducts.map((p) => (
                      <tr key={p.id} className={!p.is_active ? "adm-row-inactive" : ""}>
                        ...
                        <td className="adm-td-actions">
                          <button className="adm-btn-edit" onClick={() => openEditProduct(p)}>✏️ Editar</button>
                          <button
      ...
      >
        ...
        {isPending && (
          <div className="adm-order-actions" onClick={(e) => e.stopPropagation()}>
            <button className="adm-btn-accept" onClick={onAccept} disabled={isUpdating}
          ...
          <div className="adm-detail-section">
            ...
            {isPending ? (
              <div className="adm-accept-reject-detail">
                ...
                <button className="adm-btn-reject-lg" onClick={onReject} disabled={isUpdating}>
                  ✕ Rejeitar Pedido
                </button>
              </div>
            ) : (
