...
const DELIVERY_STATUS_ORDER = ["pending", "preparing", "on_the_way", "delivered"];
...
const CATEGORIES = ["cerveja", "vinho", "destilado", "refrigerante", "energetico", "outros"];

const EMPTY_PRODUCT = {
  id: "", name: "", category: "cerveja", price: "",
  old_price: "", image: "", stock: "", is_active: true,
  promotion: false, supplier: "", ean: "",
};

// ── Helpers ───────────────────────────────────────────
const isPickup  = (order) => order.address?.isRetirada === true;
const getConfig = (order) => (isPickup(order) ? STATUS_PICKUP : STATUS_DELIVERY)[order.status] ?? STATUS_DELIVERY.pending;
const getStatuses = (order) => isPickup(order) ? PICKUP_STATUS_ORDER : DELIVERY_STATUS_ORDER;
...
const metricsReducer = (_, action) => ({ count: action.count, total: action.total });
...
function useVariableVirtualList(count, estimatedItemHeight = 90, overscan = 3) {
  ...
  let start = 0;
  while (start < count - 1 && offsets[start + 1] <= scrollTop - overscan * estimatedItemHeight) start++;
  let end = start;
  while (end < count - 1 && offsets[end] <= scrollTop + viewH + overscan * estimatedItemHeight) end++;

  const measureRef = useCallback((index) => (el) => {
    if (observersRef.current[index]) { observersRef.current[index].disconnect(); delete observersRef.current[index]; }
    if (!el) return;
    const ro = new ResizeObserver(([e]) => {
      const h = Math.round(e.contentRect.height);
      if (heightsRef.current[index] !== h) { heightsRef.current[index] = h; forceUpdate(); }
    });
    ro.observe(el);
...
function playNotificationSound() {
  try { const a = new Audio("/notification.mp3"); a.volume = 1; a.play().catch(() => {}); } catch {}
}
...
export default function Admin({ user, isAdmin }) {
  ...
  const [activeTab, setActiveTab] = useState("pedidos"); // ← NOVO
  ...
  const [productModal, setProductModal]   = useState(null); // null | "new" | produto
  ...
  const openEditProduct = (product) => {
    setModalForm({ ...product, price: product.price, stock: product.stock, old_price: product.old_price ?? "" });
    setModalError("");
  ...
  const handleModalChange = (e) => {
    ...
    setModalForm((prev) => ({ ...prev, [name]: type === "checkbox" ? checked : value }));
  };
  ...
  const handleModalSave = async (e) => {
    ...
    const row = {
      ...
      old_price: modalForm.old_price !== "" ? Number(modalForm.old_price) : null,
  ...
  const fetchTodayMetrics = useCallback(async () => {
    ...
    const { data, count, error } = await supabase
      .from("orders").select("total", { count: "exact" })
      .gte("created_at", today.toISOString())
      .lt("created_at", tomorrow.toISOString());
    if (!error && data) dispatchMetrics({ count: count ?? 0, total: data.reduce((s, o) => s + (o.total ?? 0), 0) });
  }, []);

  // ── Fetch pedidos ─────────────────────────────────
  const fetchOrders = useCallback(async (pageNum = 0, reset = false) => {
    pageNum === 0 ? setLoading(true) : setLoadingMore(true);
    const from = pageNum * PAGE_SIZE;
    const to   = from + PAGE_SIZE - 1;
    let query = supabase
      .from("orders")
      .select(`id, total, payment_method, address, status, created_at, order_items ( id, name, price, quantity )`, { count: "exact" })
      .order("created_at", { ascending: false })
      .range(from, to);
    if (filterStatus !== "all") query = query.eq("status", filterStatus);
    const { data, error, count } = await query;
    if (!error) {
      setOrders((prev) => reset || pageNum === 0 ? (data ?? []) : [...prev, ...(data ?? [])]);
  ...
  useEffect(() => { if (!isAdmin) return; fetchOrders(0, true); fetchTodayMetrics(); }, [isAdmin, fetchOrders, fetchTodayMetrics]);

  useEffect(() => {
    if (!isAdmin) return;
    const channel = supabase.channel("admin-orders")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "orders" }, () => {
        playNotificationSound(); fetchOrders(0, true); fetchTodayMetrics();
      }).subscribe();
  ...
  useEffect(() => {
    if (!isAdmin) return;
    setPage(0); setOrders([]); setHasMore(true); fetchOrders(0, true);
  ...
  const confirmReject = useCallback(async () => {
    ...
    const { error: itemsErr } = await supabase.from("order_items").delete().eq("order_id", rejectModal);
    if (itemsErr) { setRejectError("Erro ao remover itens."); setRejecting(false); return; }
    const { error: orderErr } = await supabase.from("orders").delete().eq("id", rejectModal);
    if (orderErr) { setRejectError("Erro ao rejeitar pedido."); setRejecting(false); return; }
    setOrders((prev) => prev.filter((o) => o.id !== rejectModal));
    setTotalCount((prev) => prev - 1);
    setRejectModal(null); setRejecting(false);
  }, [rejectModal]);

  const closeRejectModal = useCallback(() => { if (rejecting) return; setRejectModal(null); setRejectError(""); }, [rejecting]);
  const handleLoadMore   = useCallback(() => { const next = page + 1; setPage(next); fetchOrders(next); }, [page, fetchOrders]);

  const counts = orders.reduce(
    (acc, o) => { acc.all++; if (o.status in acc) acc[o.status]++; return acc; },
    { all: 0, pending: 0, preparing: 0, on_the_way: 0, delivered: 0 }
  );

  const { containerRef, totalHeight, offsets, start, end, measureRef } = useVariableVirtualList(orders.length, 90, 3);

  // ── Guards ────────────────────────────────────────
  if (isAdmin === null) return (
    <div className="adm-root"><div className="adm-wrap"><div className="adm-loading"><div className="adm-spinner" /><p>Verificando acesso...</p></div></div></div>
  );
      ...
      <div className="adm-wrap">
          ...
          <>
            ...
            <div className="adm-modal" role="dialog" aria-modal="true">
              ...
              <p className="adm-modal-desc">
                ...
                <strong>#{rejectModal.slice(-8).toUpperCase()}</strong>? Será <strong>apagado permanentemente</strong>.
              </p>
              {rejectError && <div className="adm-modal-error">⚠️ {rejectError}</div>}
              <div className="adm-modal-actions">
                <button className="adm-modal-btn-back" onClick={closeRejectModal} disabled={rejecting}>Cancelar</button>
                <button className="adm-modal-btn-reject" onClick={confirmReject} disabled={rejecting}>
              ...
              <form onSubmit={handleModalSave} className="adm-product-form">
                <div className="adm-form-row">
                  <div className="adm-form-field">
                    ...
                    <input name="id" value={modalForm.id} onChange={handleModalChange}
                      disabled={productModal !== "new"} placeholder="ex: 000468" required />
                  </div>
                  <div className="adm-form-field">
                ...
                <div className="adm-form-field">
                  <label>Nome</label>
                  <input name="name" value={modalForm.name} onChange={handleModalChange} placeholder="Nome do produto" required />
                </div>
                ...
                <div className="adm-form-row">
                  <div className="adm-form-field">
                    ...
                    <input name="price" type="number" step="0.01" min="0"
                      value={modalForm.price} onChange={handleModalChange} required />
                  </div>
                  <div className="adm-form-field">
                    <label>Estoque</label>
                    <input name="stock" type="number" min="0"
                      value={modalForm.stock} onChange={handleModalChange} required />
                  </div>
                </div>

                <div className="adm-form-field">
                  <label>Imagem (URL)</label>
                  <input name="image" value={modalForm.image ?? ""} onChange={handleModalChange} placeholder="https://..." />
                </div>
                ...
                <div className="adm-modal-actions">
                  <button type="button" className="adm-modal-btn-back"
                    onClick={() => setProductModal(null)} disabled={modalSaving}>
                    Cancelar
                  </button>
        ...
        <header className="adm-header">
          ...
          <div className="adm-header-right">
            ...
            <button className="adm-btn-back" onClick={() => navigate("/")}>← Voltar à loja</button>
          </div>
        </header>

        {/* ABAS */}
        <div className="adm-tabs">
          <button className={`adm-tab ${activeTab === "pedidos" ? "adm-tab-active" : ""}`} onClick={() => setActiveTab("pedidos")}>
            📦 Pedidos {totalCount > 0 && <span className="adm-tab-badge">{totalCount}</span>}
          </button>
          <button className={`adm-tab ${activeTab === "produtos" ? "adm-tab-active" : ""}`} onClick={() => setActiveTab("produtos")}>
            🍺 Produtos {products.length > 0 && <span className="adm-tab-badge">{products.length}</span>}
          </button>
        </div>
          ...
          <>
            <div className="adm-title-row">
              <div>
                ...
                <p className="adm-subtitle">{totalCount} pedido(s) · mostrando {orders.length} · tempo real</p>
              </div>
              <div className="adm-realtime-dot" aria-label="Tempo real">
                <span className="adm-dot-pulse" /><span>Ao vivo</span>
              </div>
            </div>
            ...
            <div className="adm-today-metrics">
              ...
              <div className="adm-metric-card">
                <div className="adm-metric-icon">💰</div>
                <div className="adm-metric-content">
                  <span className="adm-metric-value">{formatBRL(metrics.total)}</span>
            ...
            <div className="adm-stats" role="group" aria-label="Filtrar por status">
              {[
                ...
                { key: "pending",    icon: "🕐",  num: counts.pending,    label: "Aguardando" },
                { key: "preparing",  icon: "👨‍🍳", num: counts.preparing,  label: "Preparando" },
                { key: "on_the_way", icon: "🛵",  num: counts.on_the_way, label: "Em entrega" },
                { key: "delivered",  icon: "✅",  num: counts.delivered,  label: "Entregue"   },
              ].map(({ key, icon, num, label }) => (
                <button key={key}
                  className={`adm-stat adm-stat-${key} ${filterStatus === key ? "adm-stat-active" : ""}`}
                  onClick={() => setFilterStatus(key)} aria-pressed={filterStatus === key}>
            ...
            {loading ? (
              <div className="adm-loading"><div className="adm-spinner" /><p>Carregando pedidos...</p></div>
            ) : orders.length === 0 ? (
              <div className="adm-empty"><p>Nenhum pedido encontrado.</p></div>
            ) : (
              <>
                <div ref={containerRef} className="adm-virtual-container">
                  <div style={{ height: totalHeight, position: "relative" }}>
                    {orders.slice(start, end + 1).map((order, relIdx) => {
                      ...
                      return (
                        <div key={order.id} ref={measureRef(absIdx)}
                          style={{ position: "absolute", top: offsets[absIdx], left: 0, right: 0 }}>
                          <OrderCard
                            order={order}
                ...
                {hasMore && (
                  <div className="adm-load-more-wrap">
                    <button className="adm-load-more" onClick={handleLoadMore} disabled={loadingMore}>
                      {loadingMore ? <><div className="adm-spinner-sm" /> Carregando...</> : `Carregar mais (${orders.length} de ${totalCount})`}
                    </button>
                  </div>
                )}
          ...
          <>
            <div className="adm-title-row">
              <div>
                ...
                <p className="adm-subtitle">{filteredProducts.length} de {products.length} produtos</p>
              </div>
            ...
            {productsLoading ? (
              <div className="adm-loading"><div className="adm-spinner" /><p>Carregando produtos...</p></div>
            ) : (
...
// ── Card de pedido (sem alteração) ────────────────────
function OrderCard({ order, isExpanded, isUpdating, onToggle, onAccept, onReject, onAdvance, onSetStatus }) {
  ...
  return (
    <li className={`adm-order adm-order-${order.status}`}>
      <div className="adm-order-main" onClick={onToggle} role="button" tabIndex={0} onKeyDown={(e) => e.key === "Enter" && onToggle()}>
        <div className="adm-order-status">
          <span className="adm-status-icon">{cfg.icon}</span>
          <span className="adm-status-label" style={{ color: cfg.color }}>{cfg.label}</span>
        ...
        {isPending && (
          <div className="adm-order-actions" onClick={(e) => e.stopPropagation()}>
            <button className="adm-btn-accept" onClick={onAccept} disabled={isUpdating} title={pickup ? "Confirmar retirada" : "Aceitar pedido"}>
              {isUpdating ? "..." : `✓ ${pickup ? "Confirmar" : "Aceitar"}`}
            </button>
            <button className="adm-btn-reject" onClick={onReject} disabled={isUpdating}>✕ Rejeitar</button>
          </div>
        )}
        <span className="adm-order-date">{formatDate(order.created_at)}</span>
        <span className={`adm-chevron ${isExpanded ? "open" : ""}`} aria-hidden="true">▾</span>
      </div>

      {isExpanded && (
        <div className={`adm-order-detail adm-order-detail--${pickup ? "pickup" : "delivery"}`}>
            ...
            <div className="adm-items">
              {(order.order_items ?? []).map((item) => (
                <div className="adm-item" key={item.id}>
                  ...
                  <span className="adm-item-price">{formatBRL(item.price * item.quantity)}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="adm-detail-section">
            <div className="adm-detail-label">{pickup ? "🏪 Informações da Retirada" : "📍 Endereço"}</div>
            <div className="adm-address">
              <p><strong>{order.address?.name}</strong></p>
              {pickup ? (
                <><p>🏪 Retirada na loja</p><p>📍 Rua Edgar Torres, 650 — Belo Horizonte/MG</p></>
              ) : (
                <><p>{order.address?.street}, {order.address?.number}{order.address?.complement ? ` — ${order.address.complement}` : ""}</p><p>{order.address?.district}</p></>
              )}
              <p>📞 {order.address?.phone}</p>
            </div>
          </div>
          <div className="adm-detail-section">
            <div className="adm-detail-label">{pickup ? "🔄 Status da Retirada" : "🔄 Alterar Status"}</div>
            {isPending ? (
              <div className="adm-accept-reject-detail">
                <button className="adm-btn-accept-lg" onClick={onAccept} disabled={isUpdating}>
                  {isUpdating ? "Processando..." : `✓ ${pickup ? "Confirmar Retirada" : "Aceitar Pedido"}`}
                </button>
                <button className="adm-btn-reject-lg" onClick={onReject} disabled={isUpdating}>✕ Rejeitar Pedido</button>
              </div>
            ) : (
              <>
                <div className="adm-status-pills">
                  {statuses.map((s) => {
                    ...
                    return (
                      <button key={s} className={`adm-pill ${order.status === s ? "adm-pill-active" : ""}`}
                        style={{ "--pill-color": map[s]?.color }} onClick={() => onSetStatus(s)} disabled={isUpdating}>
                ...
                {nextSt ? (
                  <button className="adm-btn-advance" onClick={onAdvance} disabled={isUpdating}>
                    {isUpdating ? "Atualizando..." : (() => { const map = pickup ? STATUS_PICKUP : STATUS_DELIVERY; return `Avançar para: ${map[nextSt]?.icon} ${map[nextSt]?.label} →`; })()}
                  </button>
                ) : (
                  <div className="adm-delivered-msg">{pickup ? "✅ Retirada finalizada" : "✅ Pedido finalizado"}</div>
                )}
              </>
            )}
          </div>
