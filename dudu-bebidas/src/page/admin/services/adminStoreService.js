...
export async function listCategories() {
  ...
  if (error) throw new AdminServiceError("Não foi possível carregar as categorias.", error);
  return data ?? [];
}
...
export async function createCategory(name) {
  ...
  if (error) {
    if (error.code === "23505") {
      throw new AdminServiceError(`Já existe uma categoria chamada "${normalized}".`);
    }
...
export async function updateCategory(id, name) {
  const normalized = name.trim().toLowerCase();
  console.log("[categories] updateCategory →", { id, normalized });
  ...
  if (error) {
    ...
    throw new AdminServiceError("Não foi possível atualizar a categoria.", error);
  }
}
...
export async function toggleCategory(id, is_active) {
  ...
  if (error) throw new AdminServiceError("Não foi possível alterar o status da categoria.", error);
}
...
export async function deleteCategory(id, categoryName) {
  ...
  if (countError) throw new AdminServiceError("Não foi possível verificar os produtos.", countError);
  if (count > 0) {
  ...
  if (error) throw new AdminServiceError("Não foi possível excluir a categoria.", error);
}
...
export async function listDeliveryZones() {
  const { data, error } = await supabase
    ...
    .select("*")
    .order("sort_order");
  if (error) throw new AdminServiceError("Não foi possível carregar os bairros.", error);
  return data ?? [];
}

export async function createDeliveryZone({ nome, frete, is_retirada = false }) {
  if (!nome?.trim()) throw new AdminServiceError("O nome do bairro não pode estar vazio.");
  const freteNum = parseFloat(frete);
  if (isNaN(freteNum) || freteNum < 0) throw new AdminServiceError("Informe uma taxa válida.");
  ...
  if (error) throw new AdminServiceError("Não foi possível criar o bairro.", error);
  return data;
}
...
export async function updateDeliveryZone(id, { nome, frete, is_retirada }) {
  const update = {};
  if (nome !== undefined) {
    if (!nome.trim()) throw new AdminServiceError("O nome não pode estar vazio.");
  ...
  if (error) throw new AdminServiceError("Não foi possível atualizar o bairro.", error);
  return data;
}
...
export async function toggleDeliveryZone(id, is_active) {
  const { data, error } = await supabase
    ...
    .select()
    .single();
  if (error) throw new AdminServiceError("Não foi possível alterar o status.", error);
  return data;
}

export async function deleteDeliveryZone(id) {
  const { error } = await supabase.from("delivery_zones").delete().eq("id", id);
  if (error) throw new AdminServiceError("Não foi possível excluir o bairro.", error);
}
...
export async function getStoreConfig() {
  const { data, error } = await supabase
    ...
    .eq("id", 1)
    .single();
  if (error) throw new AdminServiceError("Não foi possível carregar as configurações.", error);
  return data;
}
...
export async function updateStoreConfig(config) {
  const { error } = await supabase
    ...
    .eq("id", 1);
  if (error) throw new AdminServiceError("Não foi possível salvar as configurações.", error);
}
...
export async function listStoreHours() {
  const { data, error } = await supabase
    ...
    .select("*")
    .order("day_of_week");
  if (error) throw new AdminServiceError("Não foi possível carregar os horários.", error);
  return data ?? [];
}
...
export async function upsertStoreHours(rows) {
  for (const row of rows) {
    ...
    if (error) throw new AdminServiceError(`Não foi possível salvar horário do dia ${row.day_of_week}.`, error);
  }
}
